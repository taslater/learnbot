...
class CMA {
  constructor(
    ...
    let popsize = 4 + Math.floor(3 * Math.log(this.n_dim)) // (eq. 48)
    ...
    // (eq.49)
    let weights_prime = Array(this.popsize)
    ...
    this.p_sigma = Array(this.n_dim).fill(0.0)
    // this.pc = np.zeros(n_dim)
    this.pc = Array(this.n_dim).fill(0.0)
  ...
  tell(sol_score_array) {
    ...
    let y_w = Array(this.n_dim).fill(0)
    ...
    // )
    // let temp_mat = []
    let temp_mat = Array(this.n_dim * this.popsize)
    for (let i = 0; i < this.n_dim; i++) {
      // let temp_mat_row = []
    ...
    let rank_one = Array(this.n_dim * this.n_dim)
    // upper triangle
    for (let i = 0; i < this.n_dim; i++) {
    ...
    // )
    let rank_mu = Array(this.n_dim * this.n_dim).fill(0)
    // upper triangle
    for (let i = 0; i < this.popsize; i++) {
      for (let j = 0; j < this.n_dim; j++) {
...
class SqMatrix {
  ...
  addMat(other) {
    ...
    for (let i = 0; i < this.dim * this.dim; i++) {
      add_data.push(this.data[i] + other.data[i])
    }
  ...
  mulMat(other) {
    let new_data = Array(this.dim * this.dim).fill(0)
    for (let i = 0; i < this.dim; i++) {
      for (let k = 0; k < this.dim; k++) {
        for (let j = 0; j < this.dim; j++) {
...
function eig_sym(_a) {
  let [a, d, e] = tred2(_a)
  let [eig_vals, eig_vecs] = tqli(d, e, a)
